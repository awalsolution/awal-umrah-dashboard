...
import { formRules } from '@src/rules/airLine';
...
getRecordApi(`/air-line/${props.id}`).then((res: any) => {
...
const handleValidateClick = (e: MouseEvent) => {
  ...
  formRef.value?.validate((errors) => {
    if (!errors) {
      updateRecordApi(`/air-line/${formValue.value.id}`, formValue.value).then((res: any) => {
